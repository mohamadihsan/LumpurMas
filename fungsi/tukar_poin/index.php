<?php
	...
	function BeliDenganPoin()
	{
		...
		//perhitungan poin
		$_SESSION['poin_pelanggan'] = $poin_pelanggan*1000;
		$temp_poin = $total_bayar / 100000;
		$poin = $poin_pelanggan - $temp_poin;

		if ($poin_pelanggan < $temp_poin) {
	...
	/*========================= HAPUS DATA PRODUK ========================*/
	function HapusDataTransaksiPoin()
	{
		include '../../koneksi/koneksi.php';
		...
		if ($id_pelanggan!=null) {
			...
			$plus_poin = ($total_bayar/100000) + $poin;

			//update poin pelanggan
			$sql = "UPDATE pelanggan SET poin = ? WHERE id_pelanggan = ?";
			$stmt = $db->prepare($sql);
			$stmt->bind_param('ii', $plus_poin, $id_pelanggan);
			$stmt->execute();
			$stmt->close();
		}
